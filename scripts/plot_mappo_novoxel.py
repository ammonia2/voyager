...
def main():
    parser = argparse.ArgumentParser(description="Plot MAPPO (no-voxel ablation) predator/prey JSONL logs")
    parser.add_argument(
        "--predator-log",
        default="checkpoints/mappo_novoxel_predator_metrics.jsonl",
        help="Path to predator JSONL log",
    )
    parser.add_argument(
        "--prey-log",
        default="checkpoints/mappo_novoxel_prey_metrics.jsonl",
        help="Path to prey JSONL log",
    )
    parser.add_argument(
        "--out-dir",
        default="plots/mappo_novoxel",
    ...
    # Step-level predator metrics (MAPPO aliases for PPO losses + entropy)
    if pred_step:
        pred_update = [_record_index(r) for r in pred_step]
        predator_policy_loss = [
            _first_float(r, ["policyLoss", "policy_loss", "actorLoss", "loss"]) for r in pred_step
        ]
        predator_value_loss = [
            _first_float(r, ["valueLoss", "value_loss", "criticLoss", "loss"]) for r in pred_step
        ]
        predator_entropy = [
    ...
    print(f"Saved MAPPO (no-voxel) plots to: {out_dir.resolve()}")
