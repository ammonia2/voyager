...
def main():
    parser = argparse.ArgumentParser(description="Plot MAPPO predator/prey JSONL logs")
    parser.add_argument(
        "--predator-log",
        default="checkpoints/mappo_predator_metrics.jsonl",
        help="Path to predator JSONL log",
    )
    parser.add_argument(
        "--prey-log",
        default="checkpoints/mappo_prey_metrics.jsonl",
    ...
    if pred_step:
        ...
        _save_line_plot(plt, pred_update, [_safe_float(r.get("omLoss")) for r in pred_step],
                        "Predator OM Loss", "Update", "OM Loss", out_dir / "predator_om_loss.png")
    ...
    if prey_step:
        ...
        _save_line_plot(plt, prey_update, [_safe_float(r.get("advantageMseSanity")) for r in prey_step],
                        "Prey Advantage MSE Sanity", "Update", "MSE", out_dir / "prey_advantage_mse.png")
    ...
    print(f"Saved MAPPO plots to: {out_dir.resolve()}")
