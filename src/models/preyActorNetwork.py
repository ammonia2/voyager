...
class PreyActorNetwork(nn.Module):
    """
    MAPPO actor for the prey agent.
    Action space: move (discrete, 3) + turn (discrete, 3).
    Prey NEVER attacks — no attack head.
    """
    ...
    def sampleAction(self, flatObs: torch.Tensor):
        """
        Sample discrete move + turn indices.
        returns: mIdx, tIdx, logP (scalar), entropy (scalar)
        """
